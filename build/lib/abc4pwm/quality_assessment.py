...
class ClusterQuality():
    def __init__(self, input_path_to_folder_of_DBDs, out_path_for_qa_clusters, output_folder_for_text_report,
        ...
        if load_new_assesment:
            ...
            quality_df = self.cluster_quality(self, input_path_to_folder_of_DBDs, output_folder_for_text_report,
                                              path_for_quality_assessment_file)
        else:
            if os.path.exists(os.path.join(path_for_quality_assessment_file, 'quality_df.json')):
                ...
                print(
                    "Quality assesment file is not available. Please generate assesment by setting load_new_assesment = True")
                exit()

        print("Now you can see clustering summary and quality in ", self.output_folder_for_text_report,
              " in cluster_quality.txt \n"
    ...
    def cluster_quality(self, input_folder, output_folder, path_for_quality_assessment_file):
        ...
        cols = ['dbd', 'cluster_number', 'mean', 'median', 'min', 'max', 'std', 'total_pwms', 'uncertain_pwms',
                'similarities']
        quality_df_data = []
        ...
        with open(os.path.join(path_to_text_reports, 'cluster_quality.txt'), 'w') as f:
            ...
            f.writelines(str(self.z_score_threshold) + " : Z_score \n")
            f.writelines(str(self.mean_threshold) + " : Mean\n")
            f.writelines(str(self.top_occurrence) + " : Top Occurances\n")
            f.writelines(str(self.occurrence_threshold) + " : Occurances Threshold\n\n")
            ...
            for dbd in dbds:
                ...
                clusters_list = [i for i in sorted(os.listdir(cluster_path)) if
                                 os.path.isdir(os.path.join(cluster_path, i))]
                        ...
                        if uper_triangle_similarity_matrix.mean() > self.mean_threshold:
                            ...
                            if uper_triangle_similarity_matrix.std() > 0.0:
                                ...
                                bad_pwms_indexes = self.get_items_upto_count(Counter(potential_bad_pwms),
                                                                             int(np.ceil(
                                                                                 len(pwms_full) * self.occurrence_threshold)),
                                                                             int(np.ceil(
                                                                                 len(pwms_full) * self.top_occurrence)))
