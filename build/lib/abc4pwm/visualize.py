...
class Visualize():
    def __init__(self, path_to_folder_of_assessment_file, path_to_folder_of_DBDs, output_folder, dbd_for_plot,
                 task='boxplot'):
        """
    ...
    def print_boxplot_dbd(quality_df, plot_dbd, output_path):
        ...
        fig, ax1 = plt.subplots(nrows=1, ncols=1, figsize=(24, 12), dpi=200)
        ...
        ax1.set_title('Rectangular box plot for ' + str(dbd_to_plot), fontsize=20, weight='bold')
        ax1.set_xlabel('Clusters', fontsize=18)
        ax1.set_ylabel('Observed Similarities', fontsize=18)
        ax1.yaxis.grid(True)
...
if __name__ == "__main__":
    Visualize('../data/in/', '../data/out/clustering_out/', '../data/out/plots/boxplots/',
              '../data/out/clustering_out/bZIP/')
