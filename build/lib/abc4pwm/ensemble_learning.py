...
class EnsembleLearning():
    def __init__(self, opt_dependence=0,
        ...
        if opt_numOfWeakReads and opt_numOfWeakReads > 0:
            ...
            print("Within " + len(weak_genes) + " reads, there are " + len(
                new_weak_genes) + " reads in the new weak exp files \n")
        ...
        for i in range(1, numP + 1):
            all_processes.extend(
                self.random_run_bayesPI(i, select_random_n, opt_numOfWeakReads, opt_seqFile, opt_strong_expFile,
                                        opt_iteration, opt_loops, opt_normalization,
                                        opt_min_L, opt_max_L, opt_p_value, opt_out,
                                        new_weak_genes, strong_genes, new_weak_exp,
                                        strong_exp, strong_head, opt_strand, opt_dependence,
        ...
        for job in range(1, numP + 1):
            temp_name = "_".join(
                [str(opt_strong_expFile), str(select_random_n), "plus_weak", str(opt_numOfWeakReads), str(job)])
            del temp_name
    ...
    def random_run_bayesPI(self, job_loops, opt_numOfStrongReads, opt_numOfWeakReads, opt_seqFile, opt_strong_expFile,
        ...
        outExp = "_".join(
            [str(opt_strong_expFile), str(opt_numOfStrongReads), "plus_weak", str(opt_numOfWeakReads), str(job_loops)])

        self.export_tag_counts(outExp, strong_head, all_new_keys, strong_exp, new_weak_exp)

        opt_numP = opt_max_L - opt_min_L + 1
        # added wang
        return self.run_bayesPI_parallel_by_motifLength(opt_dependence, opt_numP, outExp, opt_seqFile, opt_loops,
                                                        opt_iteration, opt_normalization, opt_out, opt_min_L, opt_max_L,
                                                        opt_p_value, opt_strand, max_processors=max_processors)

    def run_bayesPI_parallel_by_motifLength(self, opt_dependence, opt_numP, opt_expFile, opt_seqFile, opt_loops,
                                            opt_iteration, opt_normalization, opt_out, opt_min_L, opt_max_L, opt_strand,
                                            opt_p_value=0.0001, flank_sequence=0, max_evidence='', max_processors=0):
        ...
        print("min_L= " + str(opt_min_L) + ", max_L= " + str(opt_max_L) + ", numP=" + str(opt_numP) + ", in " + str(
            num_of_motifs_in_each_process))
        ...
        for job_loops in range(opt_numP):
            ...
            tmp_cmd = self.run_bayesPI_in_motif_parallel(job_loops, opt_seqFile, opt_expFile, opt_iteration, opt_loops,
                                                         opt_normalization,
                                                         opt_dependence, temp_min_L, temp_max_L, opt_p_value, opt_out,
                                                         opt_strand, opt_max_evidence, opt_flank_sequence)
            print(tmp_cmd)
            # added wang
    ...
    def run_bayesPI_in_motif_parallel(self, job_loops, opt_seqFile, opt_expFile, opt_iteration, opt_loops,
                                      opt_normalization, opt_dependence, opt_min_L, opt_max_L, opt_p_value,
                                      opt_out, opt_strand, opt_max_evidence, opt_flank_sequence):
    ...
    def read_exp_file(self, file):
        ...
        # print(hashes)
        # print(head)
        print("\t*", len(genes), " total genes in 1 experiments (missing 0 of ", len(expressions),
              " expression values; ", len(genes) - len(unique_genes), " duplicate entries")
...
if __name__ == '__main__':
    ...
    EnsembleLearning(opt_strong_expFile=expfile, opt_seqFile=seqfile, select_random_n=random_n, opt_out=out,
                     max_processors=5)
