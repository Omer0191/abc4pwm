...
class plottingCluster():
    ...
    def croping(self, img):  # cropping image
...
if __name__ == "__main__":
    # if len(sys.argv) < 2:
    #     print("Usage: python plot_cluster_motifs.py <input dir> <cluster number>")
    #     exit()
    #
    # indir = sys.argv[1]
    # # folderizeclusters(indir)
    #
    #
    # clusters = sys.argv[2:]

    # dbd_list = sorted(os.listdir('plots_c_out/'))
    # for dbd in dbd_list:
    #     if not os.path.exists(os.path.join('plots_c_out/', dbd,'out/')):
    #         continue
    #
    #
    #     indir = os.path.join('plots_c_out/',dbd,'out/')
    #     clusters = sorted(os.listdir(indir))
    #
    # for c in clusters:
    #     print(c)
    #     plotclustersummary(indir, c)
    #
    # clusterPlotObj = plottingCluster('../data/out/quality_assessed_out/HMG/out/','../data/out/plots/pdfs/', '4')
