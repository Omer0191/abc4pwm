...
def make_representative_pwm(path_to_clusters, dbd='selected', clusters='all', ic=0.4, best_match_initial_motif=1,
    ...
    if 'all' in dbd or '*' in dbd:
        ...
        for dbd in dbds:
            if os.path.exists(os.path.join(path_to_clusters, dbd, 'out/')):
                ...
                representaive_for_clusters(os.path.join(path_to_clusters, dbd, 'out/'), clusters, ic,
                                           best_match_initial_motif, mean_threshold, z_score_threshold,
...
def representaive_for_clusters(in_folder, clusters, ic, best_match_initial_motif, mean_threshold, z_score_threshold,
    ...
    for cluster in clusters:
        ...
        if len(pwms) >= 2:
            ...
            potential_rep_index = np.unravel_index(np.argmax(sum(similarityMatrix), axis=None),
