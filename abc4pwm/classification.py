...
class ClassificationPwm(object):
    def __init__(self, in_pwm_folder, out_pwm_folder, original_pwm_folder, new_database=0):
        ...
        if self.load_new_db:
            ...
            cleanTFDB(self.input_folder_path, 1)
        else:
    ...
    def classify(self):
        ...
        for i in pwms:
            ...
            for j in range(len(self.tfsdb[:, 0])):
                if tf in self.tfsdb[j, 0]:
                    tf_name_with_dbd = i[:len(i) - 4] + '-' + str(self.tfsdb[j, 1]).replace(' ', '_').replace('-',
                                                                                                              '_').replace(
                        '/', '_') + '.mlp'
                    flag = 1
        ...
        print("Task Completed. \nClassification summary can be seen in text file SummaryClassfication.txt in: ",
              path_to_text_reports)
