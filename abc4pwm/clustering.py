...
class ClusteringPwm():
    ...
    def __init__(self, input_folder_path,
        ...
        if not in_dbd:
            clusteringClassobj = non_dbd_ClusteringPwm(input_folder_path, output_folder_path, path_to_text_reports,
                                                       seed, damp, max_iter, convergence_iter, preference)
            exit()
        ...
        with open(os.path.join(path_to_text_reports, "clusterSummary.txt"), 'w') as cs:
            cs.writelines("Clustering Time: " + str(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime())) + "\n"
                                                                                                         ...
                                                                                                         "Minimum PWMs in a DBD considered for Clustering: " + str(
                self.minimum_pwms_in_dbd) + "\n\n"
                                            "Total DBDs : " + str(len(dbds)) + "\n"
                                                                               "Total Clusters made : " + str(
                self.total_clusters) + "\n"
                                       "UnClustered DBDs due to less than threshold : " + str(
                self.unclustered_dbds) + "\n"
                                         "UnClustered pwms in DBDs less than threshold: " + str(
                self.unclustered_pwms) + "\n"
                                         "Damping: " + str(damp) + "\n"
    ...
    def calculate_similarity_matrix(pwms_full, inputdir):
        ...
        for index1, i in enumerate(pwms_full):
            ...
            for index2, j in enumerate(full_pwms):
                matrix2, matrix_string2, maximum_feq2, total_maximum2, info2 = read_energy_matrix(
                    os.path.join(inputdir, j))
    ...
    def clustering(similarityMatrix, seed, damp, max_iter, convergence_iter, preference):
        ...
        clusters = AffinityPropagation(damping=damp, max_iter=max_iter, convergence_iter=convergence_iter,
                                       preference=preference, random_state=seed, affinity='precomputed',
                                       verbose=False).fit(similarityMatrix)
