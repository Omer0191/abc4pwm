...
class PWMRepresentative(object):
    ...
    def __make_reverse_strand(self, pwm):
        ''' make reverse complementary sequence but assume input matrix is 4 columns'''
        tmp = pwm.copy()
        tmp[:, 0] = pwm[:, 3]
        tmp[:, 1] = pwm[:, 2]
        tmp[:, 2] = pwm[:, 1]
    ...
    def __normalize_by_count(self, pwm, count):
        ''' mean of input matrix based on number counts'''
        tmp = pwm.copy()
    ...
    def calculate_representative(self, pwms, initial_value=None, pwms_name=None):
        ...
        if np.max(rep_count) > 0 and np.max(rev_rep_count) > 0:
            ...
            rep2, rev_rep2, rep_count2, rev_rep_count2 = self.calculate_representative4position(new_pwms,
                                                                                                potential_rep2,
                                                                                                pwm_name_list2)
