...
class PredictedCompare():
    def __init__(self, path_to_predicted_files, output_folder, db_folder, dst_for_bad_pwms, min_pwms_in_cluster=3,
                 mean_threshold=0.75, z_score_threshold=-0.9,
                 top_occurrence=0.35, occurrences_threshold=0.25, ic_for_rep=0.4, top_n=2, db_type='folder',
                 input_count=1, db_count=0, db_file_type='.txt',
                 input_file_type='.mlp', input_prob=0, db_prob=1, tf_name='', qa=1, seed=0, damp=0.5, max_iter=400,
                 convergence_iter=30, preference=None):
        """
        ...
        with open(os.path.join(path_to_text_report, 'parameters.txt'), 'w') as fp:
            ...
            fp.writelines(
                str(min_pwms_in_cluster) + " : Minimum Pwms in a cluster for results of ensembling learning and quality cluster \n")
    ...
    def make_rep(self, path_to_clusters, dbd='selected', clusters='all', ic=0.4, best_match_initial_motif=1,
                 mean_threshold=0.75, z_score_threshold=-0.9,
                 top_occurrence=0.35, occurrences_threshold=0.25):
        make_representative_pwm(path_to_clusters, dbd, clusters, ic, best_match_initial_motif, mean_threshold,
                                z_score_threshold,
    ...
    def ensembling_compare_investigate(self, path_of_clusters='out/clustering_out/out/', predicted='',
                                       min_threshold_for_number_of_pwms=3):

        clusters_list = [i for i in sorted(os.listdir(path_of_clusters)) if
                         os.path.isdir(os.path.join(path_of_clusters, i))]
    ...
    def do_searching(self, in_folder, db_folder, out_folder, tf_name, n, db_type, input_count, db_count, db_file_type,
        ...
        with open(os.path.join(path_to_text_reports, "ensemble_summary.txt"), 'w') as f:
            ...
            for ind, pwm in enumerate(pwms):
                ...
                temp_item_list, temp_list_score = searching(pwm=pwm, out=new_out_folder, tf_name=tf_name,
                                                            db_path=db_folder, db_type=db_type,
                                                            input_count=input_count,
                                                            db_count=db_count,
                                                            db_file_type=db_file_type,
                                                            input_file_type=input_file_type,
    ...
    def quality_assessment(self, cluster_path, out_path_for_qa_clusters, dst_for_bad_pwms, path_to_text_reports,
                           mean_threshold, z_score_threshold, occurrence_threshold, top_occurrence):

        copy_tree(cluster_path, out_path_for_qa_clusters)

        out_path_for_qa_clusters = os.path.join(out_path_for_qa_clusters, 'out')
        clusters_list = [i for i in sorted(os.listdir(out_path_for_qa_clusters)) if
                         os.path.isdir(os.path.join(out_path_for_qa_clusters, i))]
        clusters_list = [int(x) for x in clusters_list]
        clusters_list.sort()
...
if __name__ == '__main__':
    ...
    PredictedCompare(path_to_predicted_files=src_folder,
                     ...
                     min_pwms_in_cluster=min_pwms_in_cluster, mean_threshold=mean_threshold,
                     z_score_threshold=z_score_threshold,
